...
import { ShieldCheck, LayoutDashboard, Search, MessageSquare, Wallet, User, LogOut } from "lucide-react";

const nav = [
  { href: "/tradesperson/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/tradesperson/jobs", label: "Browse Jobs", icon: Search },
  { href: "/tradesperson/messages", label: "Messages", icon: MessageSquare },
  { href: "/tradesperson/payments", label: "Earnings", icon: Wallet },
  { href: "/tradesperson/profile", label: "My Profile", icon: User },
];

export default async function TradespersonLayout({ children }: { children: React.ReactNode }) {
  const session = await auth();
  if (!session?.user || session.user.role !== "TRADESPERSON") redirect("/sign-in");
  if (!session.user.onboardingComplete) redirect("/onboarding/tradesperson");
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-40 border-b border-border bg-background px-4 h-14 flex items-center justify-between">
        <Link href="/tradesperson/dashboard" className="flex items-center gap-2 font-bold text-primary-700">
