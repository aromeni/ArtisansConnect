...
>(({ className, children, error, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      "placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1",
      "disabled:cursor-not-allowed disabled:opacity-50 transition-colors",
      error ? "border-red-500" : "border-border hover:border-primary-400",
      className
    )}
    {...props}
  >
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-72 min-w-[8rem] overflow-hidden rounded-lg border border-border bg-background shadow-lg",
